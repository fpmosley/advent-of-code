...
'''
Advent of Code 2022 - Day 16: Proboscidea Volcanium (Part 2)
    ...
    try:
        ...
        with open(filename, "r") as file:
            ...
            flows_per_path = find_shortest_path(
                current='AA', state=0, remaining_time=26, pressure_flow=0, flow_per_path={})

            max_flow = max(v1 + v2 for k1, v1 in flows_per_path.items()
                           for k2, v2 in flows_per_path.items() if not k1 & k2)
            print(
                f"The most pressure that can be released in 26 minutes: {max_flow}")
