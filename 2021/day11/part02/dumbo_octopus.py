...
class EnergyLevelGrid():
    ...
    def increment_grid(self, increment_by=0):
        increment_grid = np.full(self._grid.shape, 1)
        self._grid = self._grid + increment_grid

    def all_flash(self):
        # Find all octopuses where energy level == 0
        result = np.where(self._grid == 0)
        octopuses = self.get_coordinates(result)

        return len(octopuses) == self._grid.size
...
def main():
    ...
    try:
        with open(filename, "r") as file:
            ...
            print(
                f"\nThe first step all octopuses flash: {step}")
            print(
                f"The total number of flashes for {step} steps: {total_num_of_flashes}")
