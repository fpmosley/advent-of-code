...
def main():
    ...
    try:
        with open(filename, "r") as file:
            ...
            # Find the largest magnitude sum
            start = time.time()

            # Note: Added in a deepcopy because the 'reduce' function modifies the iterator
            best = max(map(magnitude, map(reduce, map(
                copy.deepcopy, map(list, permutations(numbers, 2))))))
            end = time.time()

            print(
                f"Largest magnitude of any sum of two different snailfish numbers: {best}\n")
